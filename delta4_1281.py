# -*- coding: utf-8 -*-
# $Id: delta4_1281.py | Rev 45  | 2021/01/25 07:14:53 tin_fpga $
...
if cfg.get('testset', 'mode', 1) == 'delta4':
    delta_res = 4
else:
    delta_res = 0

#dmm1   = imp.load_source('k2182a', 'devices/k2182.py')                 # Load Keysight 3458A support
dmm1   = imp.load_source('d1281', 'devices/d1281.py')                 # Load Keysight 3458A support
...
dmm1 = dmm1.dat_meter (8,0,"1281")  # GPIB.
...
dmm1.set_input("CH_B")
dmm1.set_dcv_fast_range(0.1)                                                # 3458B function/range config
...
#mfcb.mfc_oper()
time.sleep(2)
dmm1.set_dcv_fast_range(0.1)
zerof1 = dmm1.get_data() #dmm1.get_adata()
zerof2 = 0#dmm2.get_data()
...
def get_dsample():
    ...
    dmm1.set_dcv_fast_range(0.1)
    s1a = dmm1.get_data()
    s2a = 0#dmm2.read_val()[1]
    s3a = 0#dmm3.read_val()[1]
    s4a = 0#dmm4.read_val()[1]
