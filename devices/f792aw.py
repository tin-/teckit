...
logfn = "f792a_raws_2v2_4920m_on_3v_filt10hz_test.log"
def log(cmd):
...
def init_source_acvxfer():
    ...
    dut   = imp.load_source('acv', 'devices/w4920m.py')                    # Load Fluke 5720+5725 support
    global mfc
    mfc = mfc1.hulk(1,0,"5720A")  # GPIB 1
    mfc.mfc_stby()
    time.sleep(1)
    mfc.mfc_out_acv(0.0, 0)  # 0 V, 0 Hz
    mfc.mfc_rangelock("OFF") # Range Lock MFC
    log("RANGE 792A = %.3f V, MFC 5720A H2 20, EXTGUARD ON, EXTSENSE ON, Direct connection via switch and Tee" % float(cfg.get('acv_xfer', 'range')) )
    ...
    global dmmu
    dmmu = dut.dat_meter(9,0,"W4920M")  # GPIB 1
    dmmu.set_acv_range(3)
...
def measure_dc_refdut(voltage):
	...
	dmmx.trigger()
        reading = dmmx.read_val()[1]
        readingu = voltage#dmmu.get_data()
        log("3458A read soak %.7f +V ; 4920M read soak %.7f +V" % (reading, readingu))
	...
	dmmx.trigger()
	reading = dmmx.read_val()[1]
        readingu = voltage#dmmu_get_data()
	volt_dcp.append(reading)
...
def measure_acv_detdut(voltage, freq):
	...
	dmmx.trigger()
        reading = dmmx.read_val()[1]
        readingu = dmmu.get_data()
	print ("\033[%d;145H\033[35m Soaking = %.7f %.7f\033[39m" % (cnt + 28, reading, readingu))
        log("3458A read soak %.7f VAC ; 4920M read soak %.7f VAC" % (reading, readingu))
        time.sleep(snooze / samplenum)
    log("Waited %d sec" % snooze)
    for cnt in range(0, samplenum):
	dmmx.trigger()
...
def run_flatness_test(xfer_range):
    ...
    acv_points = ([2.1e-3, 2.2e-3, 3e-3, 6.0e-3, 7e-3, 10e-3, 19e-3, 20e-3, 21e-3],                        # 22mV
        ...
        [300e-3, 500e-3, 600e-3, 700e-3, 0.9, 1, 1.1, 1.9, 2.0, 2.1],                           # 2.2 V
