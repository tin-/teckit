...
def init_source_acvxfer():
    ...
    global mfc
    mfc = mfc1.hulk(20,0,"5720A")  # GPIB 1
    ...
    global dmmx
    dmmx = dmm1.dmm_meter(11,0,"3458C")  # GPIB 1
...
def run_flatness_test(xfer_range):
    ...
    test = raw_input("Make sure 5720A->792A->3458A connections, switch 792A RANGE to %.3f V, make sure 792A POWERED!\r\n" % (acv_ranges_792a[sel_range]) )

    testid = 0
    if (acv_ranges_792a[sel_range] < 0.023):
		...
		if (acv_points[sel_range][vidx] * freq_low_points[fidx] > 2.2e7):
		    ...
		    continue
                dc_ref, dstdp, dstdn = measure_dc_ref( acv_points[sel_range][vidx] )
                print ("\033[0;80H\033[44;1m POINT %d, Volt %.3g V at freq %.6g Hz      \033[49;0m" % (fidx, acv_points[sel_range][vidx], freq_low_points[fidx]))
		ac_result, acsdev, mfc_unc = measure_acv_det(acv_points[sel_range][vidx], freq_low_points[fidx])
                acdc_diff_ppm = (dc_ref - ac_result) / dc_ref
		flatness_res.append(["ACV",acv_points[sel_range][vidx],"FREQ", freq_low_points[fidx],"DCM","%.9g, STD P %.3e,N %.3e" % (dc_ref, dstdp, dstdn), "ACM", "%.9g, STD, %.3e" % (ac_result, acsdev), "XFER", "%.3e" % acdc_diff_ppm, "H2U=%s" % (mfc_unc) ])
